...
public class MIDFeature : ScriptableRendererFeature
{
    ...
    class MaterialPropertyBlockData
    {
        ...
        public static void SetColorByMeshes(Renderer[] renderers)
        {
            if (renderers != null && renderers.Length > 0)
            {
                foreach (Renderer renderer in renderers)
                {
                    ...
                    else
                    {
                        ...
                        if (!renderer.TryGetComponent<MeshFilter>(out meshFilter))
                        {
                            continue;
                        }
                        if (meshFilter.sharedMesh != null)
                        {
                            renderer.GetPropertyBlock(m_MaterialPropertyBlockData.materialPropertyBlock, 0);
                            m_MaterialPropertyBlockData.materialPropertyBlock.SetColor(Shader.PropertyToID("_ColorID"), MIDManager.GetColor(meshFilter.sharedMesh, renderer.gameObject));
                            renderer.SetPropertyBlock(m_MaterialPropertyBlockData.materialPropertyBlock, 0);
                        }
                    }
                }
            }
    ...
    void SetRenders(string sceneName)
    {
        if (isActive)
        {
            if (m_SceneName == null || m_SceneName != sceneName)
            {
        ...
        switch (mode)
        {
            ...
            case MIDMode.ByMesh:
                if (m_MaterialPropertyBlockData.m_MaterialIDMode != MIDMode.ByMesh || !isActive)
                {
                    ...
                    MIDManager.Clear();
                    MaterialPropertyBlockData.SetColorByMeshes(renderers);
                }
